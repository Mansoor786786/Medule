...
def find_tesseract():
    tesseract_paths = [
        "tesseract",  # System PATH (try first)
        "/usr/bin/tesseract",  # Linux/Render standard
    ...
    for path in tesseract_paths:
        try:
            if path == "tesseract":
                ...
                print(f"✅ Tesseract found in system PATH: {result.stdout.split()[1] if result.stdout else 'unknown version'}")
                return True
            elif os.path.exists(path):
                pytesseract.pytesseract.tesseract_cmd = path
                result = subprocess.run([path, "--version"], 
                                      capture_output=True, check=True, text=True)
                print(f"✅ Tesseract found at {path}: {result.stdout.split()[1] if result.stdout else 'unknown version'}")
                return True
        except (subprocess.CalledProcessError, FileNotFoundError, IndexError) as e:
            print(f"❌ Tesseract not found at {path}: {e}")
            continue
    
    print("⚠️ Tesseract OCR not found - PDF text extraction only")
    return False
...
@app.route('/test-ocr')
def test_ocr():
    """Test endpoint to check OCR availability"""
    try:
        # Test Tesseract
        result = subprocess.run(["tesseract", "--version"], 
                              capture_output=True, check=True, text=True)
        ...
        return jsonify({
            ...
            'version': version,
            'message': '✅ Tesseract OCR is working properly'
        })
    except Exception as e:
        return jsonify({
            'tesseract_available': False,
            'error': str(e),
            'message': '❌ Tesseract OCR is not available'
        })

if __name__ == '__main__':
    print("🚀 Medical Report Analyzer Starting...")
    port = int(os.environ.get('PORT', 8000))
    print(f"� Rudnning on port: {port}")
